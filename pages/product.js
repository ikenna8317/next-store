...
import { mockSectionForProductPage } from '../mockdata'
...
function ProductPage({ dispatch, route, cartItemNames, favItemNames }) {
    const { uri, name, price, quantity } = route.params

    const inCartItems = () => {
        if (cartItemNames.indexOf(name) !== -1) {
            return true
        }
    ...
    const inFavItems = () => {
        if (favItemNames.indexOf(name) !== -1) {
            return true
        }
        return false
    }

    let itemInCart = inCartItems()
        ...
        <View style={{flex: 1}}>
            ...
            {/* <NavBar/> */}
                ...
                <View style={styles.inner}>
                    ...
                    <AddToFavoritesButton dispatch={dispatch} pid={name} productContext={{uri, name, price}} inFavItems={inFavItems}/>
                ...
                <ProductCategorySect data={mockSectionForProductPage}/>
...
const mapStateToProps = state => ({
    cartItemNames: state.cartItems.map(item => item.name),
    favItemNames: state.favItems.map(item => item.name)
})

export default connect(mapStateToProps)(ProductPage)


function AddToFavoritesButton({ dispatch, productContext, inFavItems }) {
    const [active, setActive] = useState(inFavItems)
    ...
    <TouchableOpacity style={styles.addFavBtn} onPress={() => onPress()}>
       <View style={styles.addFavBtnInner}>
           <MaterialIcons name="favorite-outline" size={24} color="#FF4487" />
    ...
    <View style={styles.addFavBtn}>
        <View style={styles.addFavBtnInner}>
            <MaterialIcons name="favorite" size={24} color="#FF4487" />
            <Text style={{color: '#cecece', fontFamily: 'OpenSans_400Regular', marginLeft: 8, fontSize: 16}}>Added</Text>
        </View>
    </View> 
